...
class BackbeatAPI {
    ...
    isValidRoute(bbRequest) {
        ...
        // TODO: Remove monitoring/metrics later on
        if (shortRoute === 'metrics' || shortRoute === 'monitoring/metrics') {
            if (bbRequest.getHTTPMethod() !== 'GET') {
                return errors.MethodNotAllowed;
            }
            bbRequest.setContentType(ReplicationMetrics.asPrometheusContentType());
            return true;
        }
    ...
    monitoringHandler(details, cb) {
        const promMetrics = ReplicationMetrics.asPrometheus();
