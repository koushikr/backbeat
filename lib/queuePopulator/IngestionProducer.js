...
class IngestionProducer {
    ...
    constructor(sourceConfig, qpConfig, s3Config) {
        console.log('Ingestion producer constructor params--->');
        console.log('sourceConfig, qpConfig, s3Config---->', sourceConfig, qpConfig, s3Config);
    ...
    _getObjectVersionsList(bucket, state, done) {
        ...
        console.log('req-->', req);
        // console.trace('IngestionProducer._getObjectVersionsList');
    ...
    _getObjectMetadata(bucket, key, versionId, done) {
        ...
        attachReqUids(req, this.requestLogger);
        req.send((err, blob) => {
            // console.log('_getObjectMetadata------>', blob);
            if (err) {
    ...
    _getBucketCseq(bucket, done) {
        ...
        (err, data) => {
            // console.log('get bucket cseq------->', data);
            if (err) {
