...
class KafkaBacklogMetrics extends EventEmitter {
    ...
    _getLatestTopicOffset(kafkaClient, topic, partition, cb) {
        this._log.debug('querying latest partition offset from zookeeper', {
            topic, partition,
        });
    ...
    _checkConsumerOffsetsGeneric(topic, groupId, maxLag, snapshotName, cb) {
        ...
        this._zookeeper.getChildren(partitionsZkPath, (err, partitions) => {
            if (err) {
                if (err.getCode() === zookeeper.Exception.NO_NODE) {
                    // no consumer published offsets yet
                    return cb();
                }
                this._log.error(
                ...
                async.waterfall([
                    ...
                    (offset, next) => {
                        ...
                        consumerOffsets.forEach(consumerOffsetInfo => {
                            const lag = topicOffset - consumerOffsetInfo.offset;
     ...
     * group.
     *
     * The function will return the lag last published by a call to
     * KafkaBacklogMetrics.publishConsumerBacklog().
     *
     * @param {string} topic - topic name
